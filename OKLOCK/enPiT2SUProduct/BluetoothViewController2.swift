...
//  enPiT2SUProduct
//
//  Created by Kase Yudai on 2019/12/04.
...
var mainPeripheral2: CBPeripheral? = nil
var sensor2 = String()
var leafonyID = "leafony"
...
//                leafonyID = "\(peripheral!.identifier)"
                leafonyID = peripheral!.name!
                keyID.append(leafonyID)
                mainPeripheral = self.peripheral
//                databaseRef.child("leafony/key/ID/id").setValue(keyID)
            }
        }
    }
    var monitoringTarget: Monitor = .angle
    var monitoringTarget2: Monitor = .luminous
...
//        //leafonyのIDを取得（電気の場合）
//        databaseRef.child("leafony/key/ID").observe(.childAdded, with: { snapshot in
...
//    }
    ...
    @IBAction func showPeripheralList() {
        if let peripheral = self.peripheral, peripheral.state == .connected {
            ...
            if let vc = self.storyboard?.instantiateViewController(withIdentifier: "PeripheralListViewController2") as? PeripheralListViewController {
                vc.delegate = self
...
extension BluetoothViewController2: PeripheralListViewControllerDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic.uuid.uuidString == BluetoothUUID.sensor.rawValue,
            ...
            if let sensorValue = self.getSensorValueFrom(string: string, target: self.monitoringTarget) {
                ...
                databaseRef.child(homecode).child("leafonyValue/keySensor/sensorValue").setValue(intSensorValue)
                self.unit.text = unit.toString()
            }
            if let sensorValue = self.getSensorValueFrom(string: string, target: self.monitoringTarget2) {
                sensor = sensorValue
            }
        }
    }
...
extension BluetoothViewController2 {
    @objc
